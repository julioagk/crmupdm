...
router.post('/registrar-actividad', [auth, esProspector], async (req, res) => {
    try {
        ...
        // Reglas automáticas basadas en la acción realizada y la etapa actual
        ...
        if (tipo === 'llamada' && resultadoFinal === 'exitoso') {
            // Llamada exitosa: si está en prospecto_nuevo → en_contacto
            ...
            // Mensaje exitoso: si está en prospecto_nuevo → en_contacto
            if (etapaActual === 'prospecto_nuevo') nuevaEtapaAuto = 'en_contacto';
        } else if (tipo === 'cita' && resultadoFinal === 'exitoso') {
            // Cita agendada (exitoso al agendar): → reunion_agendada si aún no llegó ahí
            ...
            // Cita realizada/convertida: → reunion_realizada
        ...
        // La etapaEmbudo explícita del body puede sobrescribir la auto-detección
        // pero solo si avanza (o si es 'perdido')
        let nuevaEtapa = etapaEmbudo || nuevaEtapaAuto;
        // Si se envió explícitamente y es igual o retrocede, ignorar
        if (nuevaEtapa && nuevaEtapa !== 'perdido') {
            const rankNueva = ORDEN_ETAPAS.indexOf(nuevaEtapa);
            if (rankNueva !== -1 && rankNueva <= rankActual) nuevaEtapa = null; // no permitir retroceder
        }
...
router.get('/estadisticas', [auth, esProspector], async (req, res) => {
    try {
        const prospectorId = parseInt(req.usuario.id);
        const clientes = await db.prepare('SELECT * FROM clientes WHERE prospectorAsignado = ?').all(prospectorId);
        const actividades = await db.prepare('SELECT * FROM actividades WHERE vendedor = ?').all(prospectorId);
...
// POST /api/prospector/importar-csv - Importacion masiva de prospectos
    ...
    try {
        ...
        for (const p of prospectos) {
            try {
                ...
                // Verificar duplicado por telefono (mismo prospector)
                if (p.telefono) {
                ...
                await db.prepare(
                    INSERT INTO clientes (nombres, apellidoPaterno, apellidoMaterno, telefono, correo, empresa, notas, etapaEmbudo, prospectorAsignado, fechaRegistro, fechaUltimaEtapa)
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'prospecto_nuevo', ?, ?, ?)
                ).run(nombres, apellidoPaterno, apellidoMaterno, telefono, correo, empresa, notas, prospectorId, ahora, ahora);
